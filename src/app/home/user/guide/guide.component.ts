...
export class GuideComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.databaseService.getGuides().then(res=>{
      this.guides = res.docs.map(doc=>{return {...doc.data(),id:doc.id}})
    })
  }
  onDelete(){
  ...
  addNewAgent(){
    const dialog = this.dialog.open(NewGuideComponent);
    dialog.closed.subscribe(res => {
      if(res){
        this.guides.push(res)
        this.databaseService.addGuide(res).then(res=>{
          console.log(res)
