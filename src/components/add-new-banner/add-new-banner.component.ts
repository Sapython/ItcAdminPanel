  ...
  async submit() {
    if (this.newBannerForm.status == 'VALID' && this.imageFile) {
      ...
      if (this.data && this.data.mode == 'add') {
        // handle image upload
        this.databaseService.addBanner(this.newBannerForm.value).then((res: any) => {
        ...
        this.dataProvider.pageSetting.blur = true
        this.databaseService.updateBanner(this.data.id, this.newBannerForm.value).then((res: any) => {
